...
use build_system::{BuildSystem, ExecutableSuggestion};
...
fn combine_files(files: &mut Vec<String>, files_in_dest_dir: &mut Vec<String>, is_single_package: bool) {
    if !is_single_package {
        // yes, I'm flexing, life's supposed to be fun
        if files.is_empty() {
            std::mem::swap(files, files_in_dest_dir);
        } else {
            files.extend(files_in_dest_dir.drain(..))
        }
    }
}
...
fn main() -> MultilineTerminator {
    ...
    let build_system = build_system::scan(&source_dir_path)?;
    ...
    loop {
        ...
        let mut files_in_destdir = Vec::new();
        loop {
            ...
            let is_in_destdir = if file.starts_with("/") {
                println!();
                println!("The file you specified is absolute, so we assume it's generated by the build system.");
                println!("The build system usually uses DESTDIR environment variable to choose where to place the files.");
                println!("So somewhere in the code there may be something that writes into $DESTDIR{}", file);
                println!("To package things correcty we need to know whether this is the case.");
                read_yes_no(&mut readline, "Does the build system use DESTDIR variable to configure installation directory? y for yes, n for no: ")?
            } else {
                false
            };

            if is_in_destdir {
                files_in_destdir.push(file);
            } else {
                files.push(file);
            }
        }
        ...
        while !suggested_executables.is_empty() {
            ...
            let path = match executable.csproj {
                Some(csproj) => {
                    ...
                    files_in_destdir.push(format!("/usr/lib/{}", package_name));
                    dest_path
                },
                None => {
                    ...
                    if executable.is_in_destdir {
                        files_in_destdir.push(filespec);
                    } else {
                        files.push(filespec);
                    }
    ...
    let single_package = shared_packages.len().saturating_add(executable_packages.len()).saturating_add(services.len()) == 1; 

    for package in &mut shared_packages {
        let spec_path = format!("pkg_specs/{}.sps", package.name);
        println!("Generating {} which will contain package specification...", spec_path);

        combine_files(&mut package.files, &mut package.files_in_destdir, single_package);

        atomic_serialize_toml(&spec_path, &package.as_package())?;
    }

    for package in &mut executable_packages {
        let spec_path = format!("pkg_specs/{}.sps", package.name);
        println!("Generating {} which will contain package specification...", spec_path);
    ...
    let source_spec = SourcePackage {
        ...
        section: "net",
        variants,
        build_depends: build_system.build_depends(),
        packages: &packages,
        skip_debug_symbols,
    };
